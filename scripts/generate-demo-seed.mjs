...
VALUES ('site', 'Clastro Demo', ${sql(siteUrl)}, ${sql(`${siteUrl}/images/clastro-logo.svg`)}, ${json(navigation)}, ${json(footer)}, ${json(booking)}, 'hello@clastro.local', '+64 21 000 0000', CURRENT_TIMESTAMP);`,
...
VALUES (${sql(`post-${post.slug}`)}, ${sql(post.slug)}, ${sql(post.title)}, ${sql(post.excerpt)}, ${sql(post.content)}, NULL, NULL, ${post.slug === "what-belongs-in-the-core-cms" ? 1 : 0}, 1, '2026-05-21T00:00:00.000Z', '4 min read', 'Clastro Editor', 'CMS Demo Author', ${sql(post.category)}, ${json([{ label: post.category }])}, ${sql(`${post.title} | Clastro Demo`)}, ${sql(post.excerpt)}, CURRENT_TIMESTAMP);`),
  ...
  ...products.map((product) => `INSERT INTO products (id, slug, name, price, price_label, cooling_kw, heating_kw, hero_image_url, hero_image_alt, short_description, installation_cost, is_front_page, aircon_type, meta_title, meta_description, family_code, family_name, category_slug, category_label, install_summary, brochure_label, brochure_href, overview, best_for_json, spec_notes_json, content_html, product_images_json, published, updated_at)
VALUES (${sql(`product-${product.slug}`)}, ${sql(product.slug)}, ${sql(product.name)}, NULL, ${sql(product.priceLabel)}, 0, 0, '', '', ${sql(product.shortDescription)}, 'Demo only', 1, 'generic', ${sql(`${product.name} | Clastro Demo`)}, ${sql(product.shortDescription)}, 'DEMO', 'Clastro Demo', 'cms-packages', ${sql(product.category)}, 'Demo install summary', 'Demo brochure', '#', ${sql(product.overview)}, ${json(["Testing product cards", "Testing detail pages", "Testing CMS fields"])}, ${json(["This is a generic starter record."])}, ${sql(`<p>${product.overview}</p><h2>Editable product copy</h2><p>Update this rich text from the Products tab in the admin.</p>`)}, '[]', 1, CURRENT_TIMESTAMP);`),
];
